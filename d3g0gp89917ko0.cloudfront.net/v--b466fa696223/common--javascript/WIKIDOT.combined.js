...
WIKIDOT.visuals = {
  showCenterMessage: function (c) {
    ...
    YAHOO.util.Dom.setY(
      ...
      OZONE.visuals.scrollOffsetY()
    );
    mTable.style.height = viewportHeight + "px";
    $j("#center-message").fadeIn(100);
    sDiv.style.visibility = "visible";
    mTable.style.visibility = "visible";
  },
  changeCenterMessage: function (c) {
    var b = $j("#center-message");
    b.html(c);
  },
...
WIKIDOT.utils.handleError = function (c) {
  if (c.status != "ok") {
    ...
    b.content =
      "<h1>" + ogettext("Please note:") + "</h1><p>" + c.message + "</p>";
    b.show();
    return false;
  } else {
    return true;
...
WIKIDOT.render.printuser = function (c, g, f) {
  var e =
    'href="javascript:;" onclick="WIKIDOT.page.listeners.userInfo(' + c + ')"';
  ...
  if (f == true) {
    b +=
      ...
      new Date().getTime() +
      '" alt="" style="background-image:url(' +
      d +
      "//" +
      URL_HOST +
      "/userkarma.php?u=" +
  ...
  $j(b)
    ...
    .each(function () {
      ...
      YAHOO.util.Event.addListener(
        a,
        "mouseover",
        WIKIDOT.render.fixAvatarHover.showHover
      );
    });
};
WIKIDOT.render.fixAvatarHover.showHover = function (k) {
  var r = $j("#avatar-hover-container");
  if (r.length == 0) {
    var r = $j('<div id="avatar-hover-container"></div>');
    $j("body").append(r);
  }
  if (this.hoverAvatar == null) {
    var h = this.getElementsByTagName("img")[0];
    var q = h.src.replace(/small/, "normal");
    q = q.replace(/16$/, "48");
    var n = document.createElement("a");
    var j = document.createElement("img");
    j.src = q;
    n.className = "avatar-hover";
    ...
    m.appendChild(j);
    n.appendChild(m);
    YAHOO.util.Dom.generateId(n);
    this.hoverAvatar = n;
    r.append(n);
    YAHOO.util.Event.addListener(
      n,
      "mouseout",
      WIKIDOT.render.fixAvatarHover.hideHover
    );
    YAHOO.util.Event.addListener(
      n,
      "mousemove",
      WIKIDOT.render.fixAvatarHover.mousemove
    );
  }
  ...
  var j = l.getElementsByTagName("img")[0];
  var o, c;
  var b = false;
  if (j.height == 0) {
    o = 48;
  ...
  YAHOO.util.Dom.setXY(l, [g - (c / 2 + 8), f - (o / 2 + 8)]);
  $j(this.hoverAvatar).fadeIn(100);
  l.lastAccess = new Date().getTime();
  setTimeout(
    'WIKIDOT.render.fixAvatarHover.mousemove.autoHide("' + l.id + '")',
    1000
  );
};
...
WIKIDOT.render.fixAvatarHover.mousemove.autoHide = function (d) {
  ...
  if (b.lastAccess + 3000 < c) {
    ...
    setTimeout(
      'WIKIDOT.render.fixAvatarHover.mousemove.autoHide("' + b.id + '")',
      1000
    );
  }
};
...
WIKIDOT.page.listeners = {
  editClick: function (g) {
    ...
    if ($("np-text")) {
      if ($("np-text").value != "") {
        var b = new OZONE.dialogs.ErrorDialog();
        b.content =
          'You have an active editor somewhere already and it is not possible to edit multiple elements at once.<br/><br/>(<a href="javascript:;" onclick="OZONE.visuals.scrollTo(\'' +
          WIKIDOT.Editor.editElementId +
          "');OZONE.dialog.cleanAll()\">scroll to active editor</a>)";
        b.show();
        return;
      } else {
  ...
  historyClick: function (c) {
    ...
    OZONE.ajax.requestModule(
      ...
      b,
      WIKIDOT.page.callbacks.historyClick
    );
  },
  filesClick: function (c) {
    ...
    OZONE.ajax.requestModule(
      ...
      b,
      WIKIDOT.page.callbacks.filesClick
    );
  },
  blockClick: function (c) {
    ...
    OZONE.ajax.requestModule(
      ...
      b,
      WIKIDOT.page.callbacks.blockClick
    );
  },
  moreOptionsClick: function (c) {
    ...
    b.fadeIn();
    $j("#more-options-button").html(
      $("more-options-button").innerHTML.replace(/\+/, "-")
    );
    YAHOO.util.Event.removeListener(
      "more-options-button",
      "click",
      WIKIDOT.page.listeners.moreOptionsClick
    );
    YAHOO.util.Event.addListener(
      "more-options-button",
      "click",
      WIKIDOT.page.listeners.lessOptionsClick
    );
  ...
  loginClick: function (j, b, k) {
    ...
    var f =
      ...
      "://" +
      URL_HOST +
      "/default--flow/login__LoginPopupScreen?originSiteId=" +
      h;
    ...
    var c = window.open(
      f,
      "login",
      "width=700,height=640,resizable=yes,directories=no,status=yes,menubar=yes,location=yes,scrollbars=yes"
    );
    c.focus();
  },
  createAccount: function (h, c, j) {
    ...
    var d =
      ...
      "://" +
      URL_HOST +
      "/default--flow/login__CreateAccountScreen?originSiteId=" +
      g;
    if (j) {
      d += "&invitationHash=" + j;
    }
    var b = window.open(
      d,
      "login",
      "width=700,height=680,resizable=yes,directories=no,status=yes,menubar=yes,location=yes,scrollbars=yes"
    );
    b.focus();
  },
  toggleEditSections: function (m) {
    if (WIKIDOT.page.vars.editSectionsActive == false) {
      ...
      for (var h = 0; h < s.length; h++) {
        ...
        o.id = "edit-section-b-" + s[h].id.replace(/toc/, "");
        YAHOO.util.Event.addListener(
          o,
          "click",
          WIKIDOT.page.listeners.editSection
        );
        $j(o).hide();
        n.insertBefore(o, s[h]);
        $j(o).fadeIn();
        f.push(o);
      }
      WIKIDOT.page.vars.editHeadings = s;
  ...
  editMetaClick: function (c) {
    ...
    OZONE.ajax.requestModule(
      "edit/EditMetaModule",
      b,
      WIKIDOT.page.callbacks.editMeta
    );
  },
  editTags: function (c) {
    var b = new Object();
    ...
    OZONE.ajax.requestModule(
      ...
      b,
      WIKIDOT.page.callbacks.editTags
    );
  },
  ...
  siteTools: function (b) {
    OZONE.ajax.requestModule(
      ...
      null,
      WIKIDOT.page.callbacks.siteTools
    );
  },
  backlinksClick: function (d) {
    ...
    OZONE.ajax.requestModule(
      ...
      c,
      WIKIDOT.page.callbacks.backlinksClick
    );
  },
  ...
  viewSourceClick: function (d) {
    ...
    OZONE.ajax.requestModule(
      ...
      c,
      WIKIDOT.page.callbacks.viewSourceClick
    );
  },
  ...
  userInfo: function (b) {
    ...
    OZONE.ajax.requestModule(
      ...
      c,
      WIKIDOT.page.callbacks.userInfo
    );
  },
  ...
  renamePage: function (c) {
    ...
    OZONE.ajax.requestModule(
      ...
      b,
      WIKIDOT.page.callbacks.renamePage
    );
  },
  ...
  createPageDiscussion: function (c) {
    ...
    OZONE.ajax.requestModule(
      "Empty",
      b,
      WIKIDOT.page.callbacks.createPageDiscussion
    );
  },
  flagPageObjectionable: function (c) {
    ...
    OZONE.ajax.requestModule(
      ...
      b,
      WIKIDOT.page.callbacks.flagPageObjectionable
    );
  },
  pageBugReport: function (b) {
    OZONE.ajax.requestModule(
      "report/BugReportModule",
      null,
      WIKIDOT.page.callbacks.pageBugReport
    );
  },
  pageRate: function (b) {
    OZONE.ajax.requestModule(
  ...
  parentClick: function (c) {
    ...
    OZONE.ajax.requestModule(
      ...
      b,
      WIKIDOT.page.callbacks.parentClick
    );
  },
  passwordRecoveryClick: function (b) {
    OZONE.ajax.requestModule(
      ...
      null,
      WIKIDOT.page.callbacks.passwordRecovery
    );
  },
  foldToc: function (b) {
  ...
  unfoldToc: function (b) {
    $j("#toc-list").fadeIn(200, function () {
      var c = $j("#toc-action-bar").find("a");
  ...
  printClick: function (c) {
    ...
    var d = window.open(
      b,
      "_blank",
      "location=no,menubar=yes,titlebar=no,resizable=yes,scrollbars=yes,width=" +
        screen.width * 0.8 +
        ",height=" +
        screen.height * 0.8 +
        ",top=" +
    ...
    OZONE.ajax.requestModule(k, c, function (e) {
      if (e.status == "no_permission") {
        OZONE.ajax.requestModule("watch/NotLoggedInModule", {}, function (o) {
          var n = new OZONE.dialogs.ActionDialog();
          n.content = o.body;
          n.show();
          return;
        });
  ...
  join: function (k, g) {
    ...
    if (g == "unified") {
      ...
      var d =
        f +
        URL_HOST +
        "/default--flow/login__JoinScreen?originSiteId=" +
        h +
        "&originUri=" +
        b.protocol +
        "//" +
      ...
      window
        .open(
          d,
          "login",
          "width=700,height=550,resizable=yes,directories=no,status=yes,menubar=yes,location=yes,scrollbars=yes"
        )
        .focus();
    }
  },
...
WIKIDOT.page.callbacks = {
  ...
  pageRate: function (b) {
    ...
    $j("#action-area")
      .html(b.body.replace(/prw54353/, "prw54354"))
      .show();
    WIKIDOT.page.utils.addCloseToActionArea();
    setTimeout("OZONE.visuals.scrollTo('action-area')", 300);
  },
...
WIKIDOT.page.utils = {
  scrollToReference: function (b) {
    OZONE.visuals.scrollTo(b, { blink: true });
  },
  addCloseToActionArea: function () {
    ...
    YAHOO.util.Event.addListener(
      b,
      "click",
      WIKIDOT.page.listeners.closeActionArea
    );
  },
  openHelpPop: function (b) {
    var c = window.open(
      "http://test.wikidot.com/default--screen/HelpPop/topic/" + b,
      "_blank",
      "location=no,menubar=no,titlebar=no,resizable=yes,scrollbars=yes,width=" +
        screen.width * 0.8 +
        ",height=" +
        screen.height * 0.8 +
        ",top=" +
...
WIKIDOT.page.fixers = {
  ...
  fixFootnoteRef: function (b) {
    ...
    for (var c = 0; c < g.length; c++) {
      ...
      var f =
        '<div class="footnote"><div class="f-heading">Footnote ' +
        j +
        '.</div><div class="f-content">' +
        d +
        '</div><div class="f-footer">(click to scroll to footnotes)</div></div>';
  ...
  fixBibRef: function (b) {
    ...
    for (var c = 0; c < e.length; c++) {
      ...
      var g =
        '<div class="reference"><div class="r-heading">Reference ' +
        j.replace(/^([0-9]+\-)/, "") +
        '.</div><div class="r-content">' +
        d +
        '</div><div class="r-footer">(click to scroll to bibliography)</div></div>';
  ...
  fixFoldableMenus: function () {
    var m = YAHOO.util.Dom.getElementsByClassName(
    ...
    for (var h = 0; h < m.length; h++) {
      ...
      YAHOO.util.Event.addListener(
        m[h],
        "click",
        WIKIDOT.page.fixers._foldableMenuToggle
      );
    }
  },
  _foldableMenuToggle: function (d) {
    ...
    if (
      !(
        YAHOO.util.Dom.hasClass(b, "folded") ||
        YAHOO.util.Dom.hasClass(b, "unfolded")
      )
    ) {
      return;
    }
...
WIKIDOT.page.init = function () {
  ...
  YAHOO.util.Event.addListener(
    "pagerate-button",
    "click",
    WIKIDOT.page.listeners.pageRate
  );
  YAHOO.util.Event.addListener(
    "tags-button",
    "click",
  ...
  YAHOO.util.Event.addListener(
    "print-button",
    "click",
    WIKIDOT.page.listeners.printClick
  );
  YAHOO.util.Event.addListener(
    "site-tools-button",
    "click",
  ...
  YAHOO.util.Event.addListener(
    "edit-meta-button",
    "click",
    WIKIDOT.page.listeners.editMetaClick
  );
  YAHOO.util.Event.addListener(
    "backlinks-button",
    "click",
  ...
  YAHOO.util.Event.addListener(
    "watchers-button",
    "click",
    WIKIDOT.page.listeners.watchersClick
  );
  YAHOO.util.Event.addListener(
    "parent-page-button",
    "click",
    WIKIDOT.page.listeners.parentClick
  );
  YAHOO.util.Event.addListener(
    "view-source-button",
    "click",
  ...
  YAHOO.util.Event.addListener(
    "page-block-button",
    "click",
    WIKIDOT.page.listeners.blockClick
  );
  YAHOO.util.Event.addListener(
    "rename-move-button",
    "click",
    WIKIDOT.page.listeners.renamePage
  );
  YAHOO.util.Event.addListener(
    "delete-button",
    "click",
  ...
  jQuery(function () {
    ...
    jQuery("#top-bar, .mark-current-link")
      .find("a[href='" + g + "'], a[href='" + c + "']")
      .addClass("current-link");
    jQuery("#discuss-button").attr(
      "href",
      jQuery("#discuss-button").attr("href") + location.hash
    );
  });
...
WIKIDOT.page.init();
(function (c) {
  c("div.wikidot-hybrid-module[id^=wikidot-hybrid-module-]").livequery(
    function () {
      var d = c(this);
      d.removeClass("wikidot-hybrid-module").addClass(
  ...
  c("div.collapsible-block a.collapsible-block-link").livequery(
    "click",
    function () {
  ...
  c(".math-equation, .math-inline").livequery(function () {
    if (!b) {
      ...
      e.src =
        "https://cdn.mathjax.org/mathjax/2.7-latest/MathJax.js?config=default";
      var f =
        'MathJax.Hub.Config({showProcessingMessages: false,jax: ["input/TeX","output/HTML-CSS"],extensions: ["tex2jax.js","MathMenu.js","MathZoom.js","Safe.js"],TeX: {  extensions: ["AMSmath.js","AMSsymbols.js","noUndefined.js", "boldsymbol.js", "mathchoice.js", "unicode.js"]},tex2jax: {  inlineMath: [ ["$","$"], ["\\(","\\)"] ],  skipTags: ["body"],  processEnvironments: true},"HTML-CSS": {  styles: {    ".MathJax_Display": { "margin": "0.5em 0"    }   }}});MathJax.Hub.Startup.onload();';
      if (window.opera) {
        e.innerHTML = f;
      } else {
        e.text = f;
    ...
    var d = function (j) {
      var h = c(j);
      if (
        h.find(".merror").length == 1 ||
        h.find('span.mtext[style~="color: red;"]:contains("\\")').length > 0 ||
        h.find(".MathJax_Error").length == 1
      ) {
        var l =
          "The equation has not been processed correctly. Click to learn more.";
        var k = c(
          '<a href="http://www.wikidot.com/doc:math" target="_blank">?</a>'
        );
...
$j(function () {
  if (
    $j("#login-status .printuser").length == 0 &&
    !$j.cookie("COOKIE_POLICY_OK")
  ) {
...
WIKIDOT.Editor.init = function (d, c, b) {
  ...
  YAHOO.util.Event.addListener(
    this.editElementId,
    "keypress",
    WIKIDOT.Editor.keyboardListener
  );
  ...
  YAHOO.util.Event.addListener(
    this.editElementId,
    "keyup",
    WIKIDOT.Editor.codeAssist.listener
  );
  OZONE.ajax.requestModule(
    "edit/EditToolbarModule",
    {},
...
WIKIDOT.Editor.initCallback = function (d) {
  ...
  if (WIKIDOT.page.vars.draft) {
    jQuery("body").scrollTo(".owindow", 0, { offset: -window.innerHeight / 2 });
  }
};
WIKIDOT.Editor.buttons = {
  bold: function (b) {
    WIKIDOT.Editor.utils.insertTags(
      "**",
      "**",
      "bold text",
  ...
  increaseListIndent: function (b) {
    WIKIDOT.Editor.utils.insertText(
      "",
      WIKIDOT.Editor.utils.increaseListIndent
    );
  },
  decreaseListIndent: function (b) {
    WIKIDOT.Editor.utils.insertText(
      "",
      WIKIDOT.Editor.utils.decreaseListIndent
    );
  },
  footnote: function (b) {
    WIKIDOT.Editor.utils.insertTags(
  ...
  bibliography: function (b) {
    WIKIDOT.Editor.utils.insertTags(
      "[[bibliography]]" + INSERT_NEWLINE + ": ",
      " : full source reference" + INSERT_NEWLINE + "[[/bibliography]]",
      "label",
      WIKIDOT.Editor.utils.trimSelection,
      WIKIDOT.Editor.utils.endWithAtLeast2NewLine,
      WIKIDOT.Editor.utils.startWithAtLeast2NewLine
  ...
  imageWizard: function (b) {
    ...
    c.content = $("wd-ed-imagewizard-dialog").innerHTML.replace(
      /\-template/g,
      ""
    );
    c.show();
  ...
  tableWizard: function (b) {
    ...
    c.content = $("wd-ed-tablewizard-dialog").innerHTML.replace(
      /\-template/g,
      ""
    );
    c.show();
  ...
  uriWizard: function (b) {
    ...
    c.content = $("wd-ed-uriwizard-dialog").innerHTML.replace(
      /\-template/g,
      ""
    );
    c.show();
  ...
  pageLinkWizard: function (f) {
    ...
    g.addButtonListener(
      "insert code",
      WIKIDOT.Editor.listeners.pageLinkWizardInsert
    );
    g.content = $("wd-ed-pagelinkwizard-dialog").innerHTML.replace(
      /\-template/g,
      ""
    );
    g.show();
    ...
    var c = new YAHOO.widget.AutoComplete(
      "wd-ed-pagelinkwizard-page",
      "autocomplete3432",
      b
    );
    c.generateRequest = function (d) {
      return (
        "?s=" + WIKIREQUEST.info.siteId + "&module=PageLookupQModule&q=" + d
      );
    };
    c.formatResult = function (d, j) {
      ...
      if (e != null) {
        return (
          "<div >" + e + '</div><div style="font-size: 85%;">(' + h + ")</div>"
        );
      } else {
        return "";
      }
  ...
  codeWizard: function (b) {
    ...
    c.addButtonListener(
      "insert code",
      WIKIDOT.Editor.listeners.codeWizardInsert
    );
    c.content = $("wd-ed-codewizard-dialog").innerHTML.replace(
      /\-template/g,
      ""
    );
    c.show();
  ...
  erefWizard: function (f) {
    ...
    g.content = $("wd-ed-erefwizard-dialog").innerHTML.replace(
      /\-template/g,
      ""
    );
    g.show();
    ...
    if (h.length == 0) {
      ...
      for (var b = 0; b < h.length; b++) {
        ...
        $("wd-ed-erefwizard-preview").innerHTML +=
          '<div id="wd-ed-erefwizard-preview-' + k + '">' + c + "</div>";
    ...
    YAHOO.util.Event.addListener(
      "wd-ed-erefwizard-ref",
      "change",
      WIKIDOT.Editor.erefWizard.changeRef
    );
...
WIKIDOT.Editor.erefWizard.changeRef = function (f) {
  ...
  $(
    "wd-ed-erefwizard-preview-" + $("wd-ed-erefwizard-ref").value
  ).style.display = "block";
};
WIKIDOT.Editor.erefWizard.insertCode = function (d) {
  ...
  WIKIDOT.Editor.ranger.setSelectionRange(
    WIKIDOT.Editor.currentPos,
    WIKIDOT.Editor.currentPos
  );
...
WIKIDOT.Editor.imageWizard.attachementSelect = function (d) {
  ...
  if (c) {
    ...
    var f =
      ...
      WIKIREQUEST.info.requestPageName +
      "/" +
      b +
      "/thumbnail.jpg";
...
WIKIDOT.Editor.imageWizard.checkFlickrImage = function (h) {
  ...
  var d = c.replace(
    /^http:\/\/(?:www\.)?flickr\.com\/.*?\/([0-9]+)(?:\/.*)?$/,
    "$1"
  );
  var b = null;
  if (
    c.match(/^http:\/\/static\.flickr\.com\/[0-9]+\/([0-9]+)_([0-9a-z]+).*$/)
  ) {
    d = c.replace(
      /^http:\/\/static\.flickr\.com\/[0-9]+\/([0-9]+)_([0-9a-z]+).*$/,
      "$1"
    );
    b = c.replace(
      /^http:\/\/static\.flickr\.com\/[0-9]+\/([0-9]+)_([0-9a-z]+).*$/,
  ...
  if (!d.match(/^([0-9]+)$/)) {
    f.innerHTML =
      '<p style="color: red">Not a valid input for the flickr.com image.</p>';
    return;
  }
  g.flickr_id = d;
  OZONE.ajax.requestModule(
    "editor/FlickrCheckModule",
    g,
...
WIKIDOT.Editor.imageWizard.checkUriImage = function (d) {
  ...
  f.document.write(
    '<html><head><title>Checking image...</title></head><body><div style="text-align: center"><p>    If you see the image below - that means the location of the image you have entered is ok.</p> <img id="check-image" src="' +
      b +
      '" alt="image not available!"/><p><a href="javascript:;" onclick="window.close()">close this window</a></p></div></body></html>'
  );
  var c = f.document.getElementById("check-image");
  YAHOO.util.Event.addListener(
    c,
    "load",
    WIKIDOT.Editor.imageWizard.checkUriImageResize,
    f
  );
};
...
WIKIDOT.Editor.imageWizard.insertCode = function (h) {
  ...
  var g = $("wd-ed-imagewizard-position")
    .value.replace(/l/, "<")
    .replace(/r/, ">")
    .replace(/c/, "=");
  var c = "[[" + g + "image " + b + n + "]]";
...
WIKIDOT.Editor.listeners = {
  ...
  codeWizardInsert: function (f) {
    ...
    WIKIDOT.Editor.ranger.setSelectionRange(
      WIKIDOT.Editor.currentRange[0],
      WIKIDOT.Editor.currentRange[1]
    );
    WIKIDOT.Editor.utils.insertTags(
      c,
      b,
...
WIKIDOT.Editor.codeAssist.listener = function (c) {
  ...
  WIKIDOT.Editor.utils.insertText(
    "",
    WIKIDOT.Editor.codeAssist.rules.listNested
  );
  WIKIDOT.Editor.codeAssist.rules.completeBlock();
  WIKIDOT.Editor.utils.insertText(
    "",
    WIKIDOT.Editor.codeAssist.rules.definitionList
  );
  WIKIDOT.Editor.utils.insertText(
    "",
    WIKIDOT.Editor.codeAssist.rules.keepIndent
  );
  WIKIDOT.Editor.utils.insertText(
    "",
    WIKIDOT.Editor.codeAssist.rules.indentEnd
  );
};
...
WIKIDOT.Editor.codeAssist.rules.completeBlock = function () {
  ...
  var d = e.length;
  e = e.replace(
    /(\[\[(code|embedvideo|math|embed)(?:\s[^\]]*?)?\]\]\r?\n)$/,
    "$1" + INSERT_NEWLINE + "[[/$2]]"
  );
  g.value = e + h;
  var j = d;
...
TextElementProxyUtil.prototype.detectBrowser = function (d) {
  if (
    YAHOO.env.ua.gecko ||
    ((this.field.selectionStart || this.field.selectionStart == 0) &&
      (!YAHOO.env.ua.ie || YAHOO.env.ua.ie >= 10))
  ) {
...
jQuery(document).ready(function () {
  jQuery(document)
    .unbind("keydown")
    .bind("keydown", "ctrl+e", function (b) {
...
function keyBindSavePage() {
  jQuery(document)
    .unbind("keydown")
    .bind("keydown", "backspace", function (b) {
